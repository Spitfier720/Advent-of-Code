...
def getAntinodes(antennaLoc, length, width):
    ...
    for i in range(len(antennaLoc)):
        for j in range(i + 1, len(antennaLoc)):
            ...
            if(inBounds((antennaLoc[i][0] - difference[0], antennaLoc[i][1] - difference[1]), length, width)):
                antinodeLoc.add((antennaLoc[i][0] - difference[0], antennaLoc[i][1] - difference[1]))
            
            if(inBounds((antennaLoc[j][0] + difference[0], antennaLoc[j][1] + difference[1]), length, width)):
                antinodeLoc.add((antennaLoc[j][0] + difference[0], antennaLoc[j][1] + difference[1]))
