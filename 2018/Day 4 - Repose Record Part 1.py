...
for e in events:
    match e[1][0]: #Primitive event handler.
        case "G": #Guard begins shift.
            #Initialize the guard with an array of 61 elements, 60 for each minute of the hour they could be sleeping,
            #and the last element to count the number of minutes they were asleep throughout our recordings of these events.
            guards.setdefault(int(e[1].split()[1].strip("#")), [0 for x in range(61)])
...
sleepiestGuard = max(list(guards.items()), key = lambda x: x[1][60])[0]
