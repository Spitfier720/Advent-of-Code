        ...
        for row in range(len(diagram)):
            for col in range(len(diagram[row])):
                if(diagram[row][col] == '@' and numNeighbors(diagram, row, col) < 4):
                    accessiblePapers += 1
                    currentRemoved.add((row, col))
        
        totalRemoved += accessiblePapers
        
        for x, y in currentRemoved:
            diagram[x][y] = '.'
...
for line in lines:
    if(line == "\n" or not line): # Empty line, reset program
        ...
        diagram.append(list(line.strip())) # Remove newline character